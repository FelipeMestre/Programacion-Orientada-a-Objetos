...
namespace CompAndDel
{
    ...
    {
        protected Propiedades Datos {get;}
        /// <summary>
        /// Filtro complejo que suaviza los bordes de una imagen.
        /// </summary>
        /// <param name="name">Nombre del objeto</param>
        public FilterConvolution(Propiedades datos)
        {
            this.Datos = datos;
        }
        /// <summary>
        ...
        private Color ObtenerColorFiltrado(Color[,] matrizVecinos)
        {
            ...
            for (int x = 0; x < matrizVecinos.GetLength(0); x++)
            {
                ...
                {
                    redFinal += matrizVecinos[x, y].R * this.Datos.Matriz[x, y]; 
                    greenFinal += matrizVecinos[x, y].G * this.Datos.Matriz[x, y]; 
                    blueFinal += matrizVecinos[x, y].B * this.Datos.Matriz[x, y];        
                }
            }
            redFinal = Math.Abs((redFinal/this.Datos.Divisor) + this.Datos.Complemento);
            redFinal = Math.Min(255, redFinal);
            
            greenFinal = Math.Abs((greenFinal / this.Datos.Divisor) + this.Datos.Complemento);
            greenFinal = Math.Min(255, greenFinal);
            
            blueFinal = Math.Abs((blueFinal / this.Datos.Divisor) + this.Datos.Complemento);
