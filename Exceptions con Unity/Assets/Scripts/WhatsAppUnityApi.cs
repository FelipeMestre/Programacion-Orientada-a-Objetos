...
public class WhatsAppMessaging : MonoBehaviour
{
	...
	public void Send(string to, string body)
	{
		...
		{
			throw new Exception("Falta el número al que mandás el mensaje");
		}
		
		if (!to.StartsWith("+598"))
		{
			throw new Exception("El número al que mandás el mensaje de comienzar con +598");
		}
		
		if (to.Length != 12)
		{
			throw new Exception("Le faltan o sobran dígitos al número al que querés mandar el mensaje");
		}
		long number;
		
		if (!Int64.TryParse(to, out number))
		{
			throw new Exception("El número al que mandás el mensaje tiene que tener sólo números");
		}
		...
		{
			throw new Exception("El parámetro callback no puede ser null");
		}
